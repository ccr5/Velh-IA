...
import { EducationTest } from '@entities/v1/education'

export class EducationTestRepository implements IAgentRepository {
  /**
  ...
  async getAgent(
    ...
    limit: string| undefined): Promise<IAgent[] | null> {
      ...
      const ret = await EducationTest.find(
  ...
  async createAgent(data: IAgent[]): Promise<IAgent[]> {
    const ret = await EducationTest.create(data)
    return ret
  }
  ...
  async updateAgent(id: string, data: IAgent): Promise<IAgent | null> {
    const ret = await EducationTest.findByIdAndUpdate(id, data)
    return ret
  }
  ...
  async deleteAgent(id: string): Promise<IAgent | null> {
    const ret = await EducationTest.findByIdAndDelete(id)
    return ret
  }
}
