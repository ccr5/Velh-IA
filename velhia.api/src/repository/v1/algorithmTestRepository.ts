import { AlgorithmTest } from '@entities/v1/algorithm'
import { IAlgorithmRepository, IAlgorithm } from '@interfaces/v1/iAlgorithm'

export class AlgorithmTestRepository implements IAlgorithmRepository {
  /**
  ...
  async getAlgorithm(
    ...
    limit: string| undefined): Promise<IAlgorithm[] | null> {
      ...
      const ret = await AlgorithmTest.find(
  ...
  async createAlgorithm(data: IAlgorithm[]): Promise<IAlgorithm[]> {
    const ret = await AlgorithmTest.create(data)
    return ret
  }
  ...
  async updateAlgorithm(id: string, data: IAlgorithm): Promise<IAlgorithm | null> {
    const ret = await AlgorithmTest.findByIdAndUpdate(id, data)
    return ret
  }
  ...
  async deleteAlgorithm(id: string): Promise<IAlgorithm | null> {
    const ret = await AlgorithmTest.findByIdAndDelete(id)
    return ret
  }
}
