...
export class ReligionTestRepository implements IAgentRepository {
  ...
  async getAgent(
    ...
    limit: string| undefined): Promise<IAgent[] | null> {
      ...
      const ret = await ReligionTest.find(
  ...
  async createAgent(data: IAgent[]): Promise<IAgent[]> {
    const ret = await ReligionTest.create(data)
    return ret
  }
  ...
  async updateAgent(id: string, data: IAgent): Promise<IAgent | null> {
    const ret = await ReligionTest.findByIdAndUpdate(id, data)
    return ret
  }
  ...
  async deleteAgent(id: string): Promise<IAgent | null> {
    const ret = await ReligionTest.findByIdAndDelete(id)
    return ret
  }
}
