...
export class FamilyTestRepository implements IAgentRepository {
  ...
  async getAgent(
    ...
    limit: string| undefined): Promise<IAgent[] | null> {
      ...
      const ret = await FamilyTest.find(
  ...
  async createAgent(data: IAgent[]): Promise<IAgent[]> {
    const ret = await FamilyTest.create(data)
    return ret
  }
  ...
  async updateAgent(id: string, data: IAgent): Promise<IAgent | null> {
    const ret = await FamilyTest.findByIdAndUpdate(id, data)
    return ret
  }
  ...
  async deleteAgent(id: string): Promise<IAgent | null> {
    const ret = await FamilyTest.findByIdAndDelete(id)
    return ret
  }
}
